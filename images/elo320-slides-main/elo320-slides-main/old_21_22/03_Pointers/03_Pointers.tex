...
Segmentos de memoria:
  \begin{itemize}
\item \textbf{Segmento de código o text}: donde programa compilado se mantiene en memoria.
\item \textbf{Memoria estática:} mantiene variables globales y estáticas. Realiza ligado estático de variables en la memoria (hasta que programa termina). 
\item \textbf{Memoria heap:} Permite crear y eliminar objetos de memoria dinámicamente y son referenciadas indirectamente mediante una variable como un \textbf{puntero} o una \textbf{referencia}.
\item \textbf{Memoria Stack o call stack:} área dinámica de memoria donde se guardan los parámetros de las funciones, información de las funciones y que permite mantener objetos que se asignan y liberan automáticamente al activar o desactivar el ambiente de ejecución.  En C son las variables. Manejado \textbf{automaticamente} por el compilador.
\end{itemize} 
}
...
\item \texttt{scanf("\%d", \&p);} Lector no sabe donde está el casillero \pause $\rightarrow$ \pause el bibliotecario le indica.
\end{itemize}
\end{frame}
...
\begin{itemize}
\item Se asigna memoria dinámicamente; específicamente desde el registro de activación, cuando se ejecuta el ámbito de declaración asociado. 
\item Se libera cuando termina su ámbito de creación.
\item Soportan bien la recursión.
\item \textbf{Ámbitos de declaración:} procedimento, función, método, bloque.
...
\frametitle{Punteros en C}
\begin{itemize}
\item Operador de desrreferencia (*): accede al valor en la dirección de memoria \textbf{a la que apunta} un puntero.
\item Los operadores * y \& son opuestos.
\item Especificador de punteros: \%p.
\item \emph{void*}: comodín para apuntar a cualquier tipo (cohersión, casting).
\begin{exampleblock}{C}
\begin{lstlisting}[language=C, basicstyle=\tiny, escapeinside={|}{|}]
int utfsm1 = 1680; //Dos variables de tipo int.
int utfsm2 = 3939; 
...
\item Nota: los punteros también tienen su propia dirección de memoria (existen!).
...
*ptr = 5.0; // * de derreferencia, se cambia el valor guardado 
	    // en la dirección de memoria puntada por ptr

b = *ptr + 1.0; // * de derreferencia, lo que hay guardado donde se apunta + 1.0 
...
\begin{itemize}
\item Preguntamos al bibliotecario o al PC de biblioteca \textbf{¿dónde está?} el libro \textbf{¿Cómo pasar EDA y FTS al mismo tiempo?}. 
...
\item \texttt{int * ubi;} \pause $\rightarrow$ \pause puntero al libro. 
\item \textbf{ubi} \pause $\rightarrow$ \pause Pasillo 3, Estante 3, Fila 1.
...
\item \textbf{Direccionamiento indirecto:} útil para diseñar estructuras (e.g. listas, árboles o grafos). ELO320 en sí. 
\item \textbf{Paso por referencia:} pasar elementos por su dirección en memoria, evitando generar una copia al llamar a una función. Ahora!
\end{itemize}
\end{itemize}
}
