...
\small
\textbf{¿Cómo podemos comparar dos algoritmos que resuelven un mismo problema?} \pause

\begin{itemize}
\item Opción 1: programar ambos y ver cuántos recursos computacionales consumen. Pero:\pause
\begin{itemize}
\item Poco eficiente en costo $\rightarrow$ sólo uno será utilizado. \pause
\item Algún algoritmo puede ser mal implementado $\rightarrow$ se subestima. \pause
\item Casos de prueba pueden favorecer a algún algoritmo en específico. \pause
\item Ambos algoritmos pueden ser muy costosos de implementar. \pause
\end{itemize}
\smallskip
\item Opción 2: Análisis de Algoritmo $\rightarrow$ Análisis de Tiempo de Ejecución.\pause
...
\item Costo for (L.3, L.4): \pause $1+(n+1)+n = 2n+2$. \pause
\item Costo L.5: \pause 1. \pause
\item Costo L.6: \pause 1. \pause
\end{itemize}
\item Costo for interno (L.4 + L.5):  \pause $2n+2+ (n \times 1) = 3n+2$. \pause
\item Costo de doble for (L.3 + L.4 + L.5): \pause $(2n+2) + n \times (3n+2) = 3n^2 + 4n + 2$. \pause
\item Costo total: \pause $3n^2 + 4n + 6$.
\end{itemize}
...
\item \textbf{Merge Sort}: $T_2(n) = c_2 \times n \times \log_2{n}$
\end{itemize}
...
\begin{itemize}
\item Paco Web programó \textbf{Insert Sort} en el PC-A usando lenguaje de máquina (assembly) resultando en un costo de $2\times n^2$ instrucciones para ordenar n números.
\item El Corxea implementa \textbf{Merge Sort}, en el PC-B usando COBOL, con un compilador antiguo e ineficiente, y el código resultante toma $50 \times n \times \log_2{n}$ instrucciones para ordenar n números.
\end{itemize}
...
\item PC-B:
$$ \frac{50 \times 10^7 \times \log_2{10^7}}{10^{7}} \approx 1163\ [s]\ \ (< 20 [min])$$
\end{itemize}
\bigskip
...
\begin{center}
\includegraphics[width=.65\linewidth]{images/timecomplexity2}
\end{center}
...
\begin{exampleblock}{Big O ($\mathcal{O}$)}\footnotesize
...
\begin{center}
\includegraphics[width=.325\linewidth]{images/bigO}
\end{center}
...
\begin{exampleblock}{Big Omega ($\Omega$)}\footnotesize
Sean $f$ y $g$ dos funciones en $\mathbb{R}$, diremos que:
...
\begin{center}
\includegraphics[width=.325\linewidth]{images/bigOmega}
\end{center}
...
\begin{exampleblock}{Big Theta ($\Theta$)}\footnotesize
Sean $f$ y $g$ dos funciones en $\mathbb{R}$, diremos que:
...
\begin{center}
\includegraphics[width=.325\linewidth]{images/bigTheta}
\end{center}
...
\pause
\begin{itemize}
\item Costo total $f(n) =$ \pause $3n^2 + 4n + 6 $.
...
\item Cota inferior: \texttt{suma} $= \Omega(n^2)$ \pause $\rightarrow$ \pause $1 \times g(n) \leq f(n) $
\item Cota promedio: \texttt{suma} $= \Theta(n^2)$ \pause $\rightarrow$ \pause $1 \times g(n) \leq f(n) \leq 10 \times g(n)$
\end{itemize}
\end{frame}
...
%\begin{center}
%\visible<.(1)>{\includegraphics[width=.35\linewidth]{images/abb-insert-example}}
%\end{center}
%\end{alertblock}
%\end{frame}
%
