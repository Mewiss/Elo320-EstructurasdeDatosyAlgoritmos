...
\item Costo total: \pause $3n^2 + 4n + 6$. \pause
\end{itemize}
...
\item El Corxea implementa \textbf{Merge Sort}, en el PC-B usando C++, con un compilador antiguo e ineficiente, y el código resultante toma $50 \times n \times \log_2{n}$ instrucciones para ordenar n números.
\end{itemize}
...
Sabemos:
\begin{itemize}
\item $T(n) = 1 + T(n-1)$ (si $n > 0$); $T(n) = 1$ (si $n = 1$).
...
\begin{itemize}
\item $T(n) = 1+ T(n/2) $ (si $n > 1$); $T(n) = 1$ (si $n = 1$).
...
\begin{itemize}
\item $T(n) = n + 2 \times T(n/2) $ (si $n > 1$); $T(n) = 1$ (si $n = 1$).
