...
\title{ELO320 - Estructuras de Datos y Algoritmos\\Introducción a C\\Estructura Base}
...
\titlegraphic{\includegraphics[scale=.145]{images/logoutfsm}}
...
Quizás la imagen anterior ahora tenga mayor sentido: 
\bigskip
\begin{figure}[h!]
\begin{center}
...
\item \textbf{codigoX.c}: es el archivo de nuestro código (puede ser más de uno).
\item \textbf{-Wall}: indica al compilador que de aviso de potenciales peligros (warnings).
\end{itemize}
}
\end{frame}
...
\item \textbf{-o} (output): indica que lo siguiente es el nombre del ejecutable. 
\item \textbf{-Wall}: indica al compilador que de aviso de potenciales peligros (warnings)
\item En la última instrucción se enlazan todos los archivos compilados, de forma independiente, en un ejecutable. 
\end{itemize}
}
\end{frame}
...
\begin{itemize}
	\item Declaración exigida para detectar potenciales errores de tipo.
	\item \textbf{Tipificación explícita (C): } 
	\begin{itemize}
		\item Siempre detecta errores de tipo. 
		\item Restricciones sobre operaciones y valores de diferentes tipos.
		\item No hay ejecución si tipos son erróneos o incompatibles. 
...
\begin{itemize}
	\item Define el tipo de dato de cada variable antes de usarlas (tipificación explícita):
	\begin{exampleblock}{C}
		\begin{lstlisting}[language=C,basicstyle=\scriptsize,escapeinside={|}{|}]
#include<stdio.h>
...
\begin{itemize}
	...
	\begin{itemize}
		\item \textbf{Verificación de tipo:} Los operandos de una operación deben ser de \emph{tipo compatible}.
...
%\vspace{-.25cm}
%\large 
\small C soporta el uso de ambos conceptos (definición y asignación) en conjunto, llamado inicialización.
\begin{exampleblock}{C}
	\begin{lstlisting}[language=C,basicstyle=\scriptsize,escapeinside={|}{|}]
#include<stdio.h>
int main(){
...
%		\item Precedencia.
%	\end{itemize}
%	\item Si \textbf{orden es ambigüo} puede conducir a \textbf{diferentes resultados}, \textbf{según la implementación} del traductor de lenguaje.
...
%\begin{block}{}
%	Consisten en conjunto de \textbf{operadores}, \textbf{operandos} (argumentos), \textbf{paréntesis} y \textbf{llamadas a funciones}. El \textbf{orden de evaluación} (precedencia, asociatividad y paréntesis) determina su \textbf{valor}. 
%\end{block}
%\bigskip
%\begin{itemize}
%	\large
...
%    #instrucciones ... 
%		\end{lstlisting}\end{block}
%		\includegraphics[width=0.8\linewidth]{images/for-python.png}
%	\end{column}
%\end{columns}
%\end{frame}
%
