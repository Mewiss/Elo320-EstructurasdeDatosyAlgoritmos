...
\title{ELO320 - Estructuras de Datos y Algoritmos\\QuickSort\\{\small Divide \& Conquer Parte II}}
...
\begin{itemize}
\item Programe y dibuje la operación \texttt{partition} sobre el arreglo $A = \{13; 19; 9; 5; 12; 8; 7; 4; 21; 2; 6; 11\}$.
...
\begin{itemize}
\item Programe la operación \texttt{partition} utilizando como critero la búsqueda de la mediana del arreglo (este no puede ser más de $\mathcal{O}(n)$).
\item Compare QuickSort utilizando diferentes versiones del método \texttt{partition}.
\end{itemize}
\end{exampleblock}
\end{frame}
...
\small Es un algoritmo de ordenamiento \textbf{no estable} de complejidad promedio \textbf{esperada} $\Theta(n \log n)$, sin embargo puede en sus peores casos llegar a $\mathcal{O}(n^2)$. Al igual que MergeSort utiliza la tecnica de Divide \& Conquer.\\
\bigskip
QuickSort es unos de los algoritmos más implementados en standards de lenguajes actualmente:
\begin{itemize}
\item Usa recursividad.
\item No usa estructuras de datos anexas.
\item Técnicas para evitar complejidad $\mathcal{O}(n^2)$ (random, mediana, etc). 
...
Para evitar eso, la elección del pivote se hace de forma aleatoria, así el tiempo \textbf{esperado} (concepto estadístico) del peor escenario es $\mathcal{O}(n \log n)$. (Nota: el peor caso sigue siendo $\mathcal{O}(n^2)$).
\end{frame}
